...
//////////////////////////////////////////////////////////////////////////
// Global pointer to the modeless preview dialog (NULL when closed)
CBlockViewDlg* g_pBlockViewDlg = nullptr;
...
void AsdkBlockView()
{
    ...
    if (g_pBlockViewDlg != nullptr)
    {
        g_pBlockViewDlg->DestroyWindow();
        // g_pBlockViewDlg is set to nullptr by PostNcDestroy
        return;
    }

    // Create and show the modeless dialog owned by the AutoCAD frame
    g_pBlockViewDlg = new CBlockViewDlg(acedGetAcadFrame());
    g_pBlockViewDlg->Create(CBlockViewDlg::IDD, acedGetAcadFrame());
    g_pBlockViewDlg->ShowWindow(SW_SHOWNOACTIVATE);
}
